...
public class ResourceConverter extends AbstractResourceContentConverter<List<Resource>, Object> {
    ...
    public List<StorageEntry> convert(String moduleId, List<Resource> resources) {
        ...
        for(Resource resource: resources){
            ...
            if(!resource.isCollection() && !"text/url".equals(resource.getMimeType()) && !"DELETED".equals(resource.getStatus()) 
                    && resource.getContentURL() != null){
                resource.setDownloadKey(
                        addExtraResource(moduleId, entries, resource.getName(), resource.getTreeParentId(), resource.getContentURL())
                );
            }
            resource.setContentURL(null);
        }
        if(resources.size() > 0){
