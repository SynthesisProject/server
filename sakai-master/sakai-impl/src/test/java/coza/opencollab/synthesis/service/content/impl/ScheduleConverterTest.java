...
import coza.opencollab.synthesis.service.content.sakai.impl.ScheduleConverter;
import coza.opencollab.synthesis.service.creator.impl.DefaultStorageService;
import coza.opencollab.synthesis.service.lms.sakai.impl.SakaiResourceHandler;
...
public class ScheduleConverterTest {
    ...
    public void testConvert() {
        ...
        List<Schedule> events = sakaiScheduleHandler.getScheduleItems(doc, "", moduleId);
        ScheduleConverter instance = new ScheduleConverter();
        instance.setStorageService(new DefaultStorageService());
        List resultStorageEntries = instance.convert(MODULE_ID, events);
        ByteArrayEntry de = (ByteArrayEntry) resultStorageEntries.get(0);
        LOG.info(new String(de.getContents() , 0 , de.getContents().length));
